...
def index():
    ...
    if request.method == 'POST':
        ...
        df = pd.read_sql('''SELECT DS_STR 
            FROM TBASES 
            WHERE NO_BASE = ? AND FL_ATIVO = 1 
            ORDER BY NO_BASE''', g.conn, params=[base])
        conexao = df['DS_STR'].to_string(index=False)
        try:
            ...
            if str(e).find('password') > 0:
                g.cur.execute('UPDATE TBASES SET FL_ATIVO = 0 WHERE NO_BASE = ?', base)
